...
def runAdEx(params):
    ...
    # run
    results["model_results"]["net"] = net.network_sim(ext_input, net_params, rec=True, standalone_dir=compile_dir)
...
def runaLN(params):
    ...
    """
    rates_exc, rates_inh, t, mufe, mufi, IA, seem, seim, siem, siim, seev, seiv, siev, siiv, return_tuple, rhs = ti.timeIntegration(params)
...
def construct_stimulus(stim="dc", duration=6000, dt=0.1, stim_amp=0.2, stim_freq=1, stim_bias=0, n_periods=0, nostim_before=0, nostim_after=0):
    ...
    if stim == "ac":
        ...
        stimulus = np.hstack(([stim_bias] * int(nostim_before / dt), np.tile(sinus_stim(stim_freq, stim_amp) + stim_bias, n_periods)))
