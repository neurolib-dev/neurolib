...
def runModels(traj=0, manual_params=None):
    ...
    if model == "aln":
        if point != "none":
            params = fp.loadpoint(
                params, point, reload_params=True, newIC=False
            )
...
def runAdEx(params):
    ...
    if not hasattr(stimulus, "__len__"):  # in case stimulus is only a number
        stimulus = [
            stimulus
        ]  # convert it to a list (the way that set_network_params expects it)
...
def runaLN(params):
    ...
    """
    rates_exc, rates_inh, t, mufe, mufi, IA, seem, seim, siem, siim, seev, seiv, siev, siiv, return_tuple, rhs = ti.timeIntegration(
        params
    )
...
def runModels_stimulus(traj=0, manual_params=None):
    ...
    params[
        "ext_exc_current"
    ] = 0  # delete external stimulus in case it was somehow before
...
def runaLN_stimulus(params):
    ...
    if params["A_sin"] > 0 and params["f_sin"] > 0:
        ...
        stimulus = construct_stimulus(
            ...
            duration=params["duration"],
            dt=params["dt"],
            stim_amp=params["A_sin"],
            stim_freq=params["f_sin"],
            nostim_before=1000 + delay,
            n_periods=1000,
        )
        params["ext_exc_current"] = stimulus
    else:
        stimulus = construct_stimulus(
            "rect", duration=params["duration"], dt=params["dt"], stim_amp=1.0
        )
        params["ext_exc_current"] = stimulus
...
def construct_stimulus(
    ...
    def sinus_stim(
        f=1, amplitude=0.2, positive=0, phase=0, cycles=1, t_pause=0
    ):
