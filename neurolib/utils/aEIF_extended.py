...
noiseBackGroundStates_LIF_OU = [(0.0049913216140622316, 0.015342881944444442), (0.0049913216140622316, 0.060536024305555547), (0.010151039399773795, 0.015342881944444442), (0.010151039399773795, 0.060536024305555547)]

noiseBackGroundStates_EIF_OU = [(0.0053945861303803695, 0.027832709418402771), (0.0053945861303803695, 0.098247612847222193), (0.013377155530318902, 0.027832709418402771), (0.013377155530318902, 0.098247612847222193)]

# 4 different background noise states (mu,sigma) in nA: (low mu, low sig), (low mu, high sig), (high mu, low sig), (high mu, high sig)
# For the LIF neuron with a white noise input and dt=0.025 ms
noiseBackGroundStates_LIF_WN_0_25 = [(0.0010399202179046496, 0.11879340277777777), (0.0083193617432371966, 0.11879340277777777), (0.0010399202179046496, 0.31870659722222222), (0.0083193617432371966, 0.31870659722222222)]

# 4 different background noise states (mu,sigma) in nA: (low mu, low sig), (low mu, high sig), (high mu, low sig), (high mu, high sig)
# For the EIF neuron with a white noise input and dt=0.025 ms
noiseBackGroundStates_EIF_WN_0_25 = [(0.0010399202179046496, 0.18350694444444443), (0.0010399202179046496, 0.39618055555555554), (0.0083193617432371966, 0.18350694444444443), (0.0083193617432371966, 0.39618055555555554)]
...
class IFModel:
    ...
    def polarizationTransfer(self, freqs):
        ...
        z = alpha + 1.0j * beta  # z2 = -z

        dummy = 1 + np.exp(-2 * z * self.L)
        denom = dummy * (Cs * w * 1.0j + Gs) + z * gi * (2 - dummy)
    ...
    def runSim(self, duration=500e-3, E_onset=100e-3, E_amp=1, E_freq=10, E_n=1000, V0=-65e-3, dt=0.1e-3, I_ext=None, I_amp=0, I_freq=0):
        ...
        IFModel._integrationIF(V, IE, nT, self.C_s, self.gL, self.EL, Isyn, dt, I_ext, self.V_reset, self.V_cut, T_refIndex)
    ...
    def computeEquivalentInputCurrentFromBSInput(self, t_BS, I_BS, V0=-65.0e-3):
        """
    ...
    def computePointNeuronImpedance(self, freqs):
        ...
        """
        return 1 / (self.gL + 2 * 1.0j * np.pi * freqs * self.C_s)
    ...
    def computeFourierCoeffFactors_I_to_V(self, freqs):
        ...
        dummy = 1 + np.exp(-2 * z * self.L)
        denom = dummy * (self.C_s * w * 1.0j + Gs) + z * gi * (2 - dummy)
...
class EIFModel(IFModel):
    ...
    def runSim(self, duration=500e-3, E_onset=100e-3, E_amp=1, E_freq=10, E_n=1000, V0=-65e-3, dt=0.1e-3):
        ...
        for i in xrange(nT - 1):
            ...
            # Non linear version
            if not self.linearVersion:
                V[i + 1] = V[i] + dt / self.C_s * (-self.gL * (V[i] - self.EL) + self.gL * self.factorExp * self.deltaT * np.exp((V[i] - self.VT) / self.deltaT) + IE[i] + Isyn)
            else:
                V[i + 1] = V[i] + dt / self.C_s * (-self.gL * (V[i] - self.EL) + self.gL * self.factorExp * self.deltaT * (1 + (V[i] - self.VT) / self.deltaT) * np.exp((V0 - self.VT) / self.deltaT) + IE[i] + Isyn)
        return t, V, E
    ...
    def computePointNeuronImpedance(self, freqs, V0=-65e-3):
        ...
        :returns:   Complex impedances
        """
        return 1 / (self.gL * (1 - self.factorExp * np.exp((V0 - self.VT) / self.deltaT)) + 2 * 1.0j * np.pi * freqs * self.C_s)
...
class aEIFModel(EIFModel):
    ...
    def runSim(self, duration=500e-3, E_onset=100e-3, E_amp=1, E_freq=10, E_n=1000, V0=-65e-3, dt=0.1e-3, I_ext=None, I_amp=0, I_freq=0, WOutput=None):
        ...
        if not self.slowLimit:
            ...
            Isyn = self.gL * (V0 - self.EL) - self.gL * self.factorExp * self.deltaT * (1 + (V0 - self.VT) / self.deltaT) * np.exp((V0 - self.VT) / self.deltaT) + aAdapt_Soma * (V0 - self.EW)
        ...
        if not self.slowLimit:  # Without the slow limit we use automatically the non-linear version
            aEIFModel._integrationNoSlowLimit(V, W, IE, nT, self.TAdapt, aAdapt_Soma, self.EW, self.C_s, self.gL, self.factorExp, self.EL, self.deltaT, self.VT, Isyn, dt, V0, I_ext, self.V_reset, self.V_cut, T_refIndex)
        elif self.linearVersion:
            aEIFModel._integrationSlowLimitLinear(V, W, IE, nT, self.TAdapt, aAdapt_Soma, self.EW, self.C_s, self.gL, self.factorExp, self.EL, self.deltaT, self.VT, Isyn, dt, V0, I_ext)
        else:
            aEIFModel._integrationSlowLimitNonLinear(V, W, IE, nT, self.TAdapt, aAdapt_Soma, self.EW, self.C_s, self.gL, self.factorExp, self.EL, self.deltaT, self.VT, Isyn, dt, V0, I_ext)
    ...
    def _integrationNoSlowLimit(V, W, IE, nT, TAdapt, aAdapt_Soma, EW, C_s, gL, factorExp, EL, deltaT, VT, Isyn, dt, V0, I_ext, V_reset, V_cut, T_refIndex):
        ...
        for i in xrange(nT - 1):
            ...
            V[i + 1] = V[i] + dt / C_s * (-gL * (V[i] - EL) - W[i] + gL * factorExp * deltaT * np.exp((V[i] - VT) / deltaT) + IE[i] + Isyn + I_ext[i])
    ...
    def _integrationSlowLimitNonLinear(V, W, IE, nT, TAdapt, aAdapt_Soma, EW, C_s, gL, factorExp, EL, deltaT, VT, Isyn, dt, V0, I_ext):
        for i in xrange(nT - 1):
            V[i + 1] = V[i] + dt / C_s * (-gL * (V[i] - EL) + gL * factorExp * deltaT * np.exp((V[i] - VT) / deltaT) - W[i] + IE[i] + Isyn + I_ext[i])

    @numba.njit
    def _integrationSlowLimitLinear(V, W, IE, nT, TAdapt, aAdapt_Soma, EW, C_s, gL, factorExp, EL, deltaT, VT, Isyn, dt, V0, I_ext):
        for i in xrange(nT - 1):
            V[i + 1] = V[i] + dt / C_s * (-gL * (V[i] - EL) + gL * factorExp * deltaT * (1 + (V[i] - VT) / deltaT) * np.exp((V0 - VT) / deltaT) - W[i] + IE[i] + Isyn + I_ext[i])
