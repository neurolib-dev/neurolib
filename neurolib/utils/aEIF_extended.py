...
noiseBackGroundStates_LIF_OU = [
    (0.0049913216140622316, 0.015342881944444442),
...
class IFModel:
    ...
    def __init__(self):
        ...
        self.rha_dend = (
            1.5
        )  # Dendritic tree membrane internal resistivity (Ohm m)
    ...
    def updateParams(self):
        ...
        self.r_m = self.rhom_dend / (
            np.pi * self.d_dend
        )  # Tangential resistance
        self.r_i = self.rha_dend / (
            np.pi * (self.d_dend / 2) ** 2
        )  # Axial resistance
    ...
    def fieldEqInputCurrent(self, V0, freqs):
        ...
        return (
            self.gL + self.C_s * 1j * 2 * np.pi * freqs
        ) * self.polarizationTransfer(freqs)
    ...
    def runSim(
        ...
        E_comp[(t > E_onset) & (t < E_offset)] = E_amp * np.exp(
            2
            * np.pi
            * E_freq
            * (t[(t > E_onset) & (t < E_offset)] - t[t > E_onset][0])
            * 1j
        )
    ...
    @numba.njit
    def _integrationIF(
        V, IE, nT, C_s, gL, EL, Isyn, dt, I_ext, V_reset, V_cut, T_refIndex
    ):
...
class EIFModel(IFModel):
    def __init__(self):
        ...
        self.linearVersion = (
            False
        )  # True if we want to use the linear approximation ( for test ), in which
    ...
    def fieldEqInputCurrent(self, V0, freqs):
        ...
        return (
            self.gL
            * (1 - self.factorExp * np.exp((V0 - self.VT) / self.deltaT))
            + self.C_s * 1j * 2 * np.pi * freqs
        ) * self.polarizationTransfer(freqs)
    ...
    def runSim(
        ...
        if not self.linearVersion:
            Isyn = self.gL * (
                V0 - self.EL
            ) - self.gL * self.factorExp * self.deltaT * np.exp(
                (V0 - self.VT) / self.deltaT
            )
        else:
            Isyn = self.gL * (
                V0 - self.EL
            ) - self.gL * self.factorExp * self.deltaT * (
                1 + (V0 - self.VT) / self.deltaT
            )
        ...
        for i in xrange(nT - 1):
            ...
            # Non linear version
            if not self.linearVersion:
                V[i + 1] = V[i] + dt / self.C_s * (
                    -self.gL * (V[i] - self.EL)
                    + self.gL
                    * self.factorExp
                    * self.deltaT
                    * np.exp((V[i] - self.VT) / self.deltaT)
                    + IE[i]
                    + Isyn
                )
            else:
...
class aEIFModel(EIFModel):
    ...
    def __init__(self):
        ...
        self.slowLimit = (
            False
        )  # True if we want to the the slow adaptation limit
    ...
    def runSim(
        ...
        """
        aAdapt_Soma = (
            self.aAdapt * np.pi * (self.d_soma * 100) ** 2
        )  # S/cm2 * area /1000 (S)
        ...
        if (
            not self.slowLimit
        ):  # Without the slow limit we use automatically the non-linear version
            aEIFModel._integrationNoSlowLimit(
                V,
                W,
                IE,
                nT,
            ...
            aEIFModel._integrationSlowLimitLinear(
                V,
                W,
                IE,
                nT,
            ...
            aEIFModel._integrationSlowLimitNonLinear(
                V,
                W,
                IE,
                nT,
    ...
    @numba.njit
    def _integrationSlowLimitNonLinear(
        V,
        W,
        IE,
        nT,
    ...
    @numba.njit
    def _integrationSlowLimitLinear(
        V,
        W,
        IE,
        nT,
...
def computeFreqDepSensitivityAndPhaseShift(
    freqs, E_amp, V0, runIFSim, corrCoefs=[], dt=0.1e-3, plot=False, **kw
):
    tOnset = 100e-3
    ...
    for f in freqs:
        ...
        argRelMinimaV = (
            idxStimStart
            + argrelextrema(V[idxStimStart:idxStimStop], np.less_equal)[0]
        )
        argRelMinimaV = argRelMinimaV[argRelMinimaV < idxStimStop - 1]

        argRelMaximaV = (
            idxStimStart
            + argrelextrema(V[idxStimStart:idxStimStop], np.greater_equal)[0]
        )
        argRelMaximaV = argRelMaximaV[argRelMaximaV < idxStimStop - 1]

        argRelMinimaE = (
            idxStimStart
            + argrelextrema(E[idxStimStart:idxStimStop], np.less_equal)[0]
        )
        ...
        if corrCoefs is not None:
            scaledField, shiftTime = Simulation.scaleAndShiftField(
                T, V, E, E_amp, f, sensitivities[-1], phases[-1], tOnset
            )
            corrCoef = np.corrcoef(
                V[T > tOnset + shiftTime], scaledField[T > tOnset + shiftTime]
            )[1, 0]
