...
class Dataset:
    ...
    def loadDataset(self, datasetName):
        ...
        self.FCs = self.loadData(
            BOLDFilenames, key="tc", apply_function=func.fc
        )
        self.FCDs = self.loadData(
            BOLDFilenames,
            key="tc",
            apply_function=func.fcd,
            apply_function_kwargs={"stepsize": 10},
        )
...
def filterSubcortical(a, axis="both"):
    ...
    subcortical_index = np.array(
        list(range(40, 46)) + list(range(74, 82)) + list(range(94, 120))
    )
