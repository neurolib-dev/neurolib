...
def add_parameters(traj, params):
    ...
    traj.simulation.model = Parameter(
        "model", params["model"], comment="Which model to run (aln/brian)"
    )

    traj.simulation.dt = Parameter(
        "dt", params["dt"], comment="I am a useful comment!"
    )  # params['dt'], 'integration time step'
    traj.simulation.duration = Parameter(
        "duration", params["duration"], comment="total simulation time [ms]"
    )
    ...
    traj.simulation.dosc_version = Parameter(
        "dosc_version", params["dosc_version"], comment="use dosc_version (1/0)"
    )
    ...
    traj.simulation.seed = Parameter(
        "seed", params["seed"], comment="Random Seed for Noise and ICs"
    )
    ...
    traj.globalNetwork.density = Parameter(
        "density",
        params["density"],
        comment="density of SC: cutoff weakest links, from 1 to 0",
    )
    traj.globalNetwork.global_delay = Parameter(
        "global_delay",
        params["global_delay"],
    ...
    traj.globalNetwork.Cmat = Parameter(
        "Cmat",
        params["Cmat"],
        comment="relative coupling strengths, \
                Cmat[i,j]: connection from jth to ith (values between 0 and 1)",
    )
    traj.globalNetwork.lengthMat = Parameter(
        "lengthMat", params["lengthMat"], comment="fiber length matrix"
    )
    ...
    traj.globalNetwork.c_gl = Parameter(
        "c_gl",
        params["c_gl"],
    ...
    traj.localNetwork.tau_ou = Parameter(
        "tau_ou", params["tau_ou"], comment="[ms]"
    )
    traj.localNetwork.sigma_ou = Parameter(
        "sigma_ou", params["sigma_ou"], comment="[mV/ms/sqrt(ms)]"
    )
    traj.localNetwork.mue_ext_mean = Parameter(
        "mue_ext_mean", params["mue_ext_mean"], comment="[mV/ms] (OU process)"
    )
    traj.localNetwork.mui_ext_mean = Parameter(
        "mui_ext_mean", params["mui_ext_mean"], comment="[mV/ms] (OU process)"
    )
    traj.localNetwork.sigmae_ext = Parameter(
        "sigmae_ext", params["sigmae_ext"], comment="[mV/sqrt(ms)]"
    )
    traj.localNetwork.sigmai_ext = Parameter(
        "sigmai_ext", params["sigmai_ext"], comment="[mV/sqrt(ms)]"
    )
    ...
    traj.localNetwork.Ke = Parameter(
        "Ke", params["Ke"], comment='"EE = IE" assumed'
    )
    traj.localNetwork.Ki = Parameter(
        "Ki", params["Ki"], comment='"EI = II" assumed'
    )

    traj.localNetwork.de = Parameter(
        "de",
        params["de"],
        comment='[ms] local constant delay, \
                                                                    "EE = IE"',
    )
    traj.localNetwork.di = Parameter(
        "di",
        params["di"],
        comment='[ms] local constant delay, \
                                                                    "EI = II"',
    ...
    traj.localNetwork.tau_se = Parameter(
        "tau_se", params["tau_se"], comment='[ms], "EE = IE"'
    )
    traj.localNetwork.tau_si = Parameter(
        "tau_si", params["tau_si"], comment='[ms], "EI = II"'
    )
    traj.localNetwork.cee = Parameter(
        "cee", params["cee"], comment="(unitless)"
    )
    traj.localNetwork.cei = Parameter(
        "cei", params["cei"], comment="(unitless)"
    )
    traj.localNetwork.cii = Parameter(
        "cii", params["cii"], comment="(unitless)"
    )
    traj.localNetwork.cie = Parameter(
        "cie", params["cie"], comment="(unitless)"
    )

    traj.localNetwork.Jee_max = Parameter(
        "Jee_max", params["Jee_max"], comment="[mV/ms]"
    )
    traj.localNetwork.Jie_max = Parameter(
        "Jie_max", params["Jie_max"], comment="[mV/ms]"
    )
    traj.localNetwork.Jei_max = Parameter(
        "Jei_max", params["Jei_max"], comment="[mV/ms]"
    )
    traj.localNetwork.Jii_max = Parameter(
        "Jii_max", params["Jii_max"], comment="[mV/ms]"
    )

    traj.localNetwork.tau_di = Parameter(
        "tau_di",
        params["tau_di"],
        comment="[ms] Distributed delay time constant, local inhibitory connection",
    )
    traj.localNetwork.tau_de = Parameter(
        "tau_de",
        params["tau_de"],
        comment="[ms] Distributed delay time constant, local excitatory connection",
    )
    ...
    traj.IC.seem_init = Parameter(
        "seem_init", params["seem_init"], comment="COMMENT"
    )
    traj.IC.seim_init = Parameter(
        "seim_init", params["seim_init"], comment="COMMENT"
    )
    traj.IC.seev_init = Parameter(
        "seev_init", params["seev_init"], comment="COMMENT"
    )
    traj.IC.seiv_init = Parameter(
        "seiv_init", params["seiv_init"], comment="COMMENT"
    )
    traj.IC.siim_init = Parameter(
        "siim_init", params["siim_init"], comment="COMMENT"
    )
    traj.IC.siem_init = Parameter(
        "siem_init", params["siem_init"], comment="COMMENT"
    )
    traj.IC.siiv_init = Parameter(
        "siiv_init", params["siiv_init"], comment="COMMENT"
    )
    traj.IC.siev_init = Parameter(
        "siev_init", params["siev_init"], comment="COMMENT"
    )
    traj.IC.rates_exc_init = Parameter(
        "rates_exc_init", params["rates_exc_init"], comment="COMMENT"
    )
    traj.IC.rates_inh_init = Parameter(
        "rates_inh_init", params["rates_inh_init"], comment="COMMENT"
    )

    # Look up tables used to integrate the local network dynamics
    traj.f_add_parameter_group(
        "LT", comment="Look-up tables containing the parameter of the aLN model"
    )
    ...
    traj.LT.precalc_r = Parameter(
        "precalc_r", params["precalc_r"], comment="Look up table"
    )
    traj.LT.precalc_V = Parameter(
        "precalc_V", params["precalc_V"], comment="Look up table"
    )
    traj.LT.precalc_tau_mu = Parameter(
        "precalc_tau_mu", params["precalc_tau_mu"], comment="Look up table"
    )
    traj.LT.precalc_tau_sigma = Parameter(
        "precalc_tau_sigma",
    ...
    traj.LT.Irange = Parameter(
        "Irange",
        params["Irange"],
        comment="Range of the mean input for the look up tables",
    )
    traj.LT.sigmarange = Parameter(
        "sigmarange",
        params["sigmarange"],
        comment="Range of the input variance for the look up tables",
    )
    traj.LT.dI = Parameter(
        "dI",
        params["dI"],
        comment="Step size of the mean input for the look up tables",
    )
    traj.LT.ds = Parameter(
        "ds",
        params["ds"],
        comment="Step size of the input variance for the look up tables",
    )
