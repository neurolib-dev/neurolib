...
def chunkwiseTimeIntAndBOLD(
    ...
    """
    Run the interareal network simulation with the parametrization params,
    compute the corresponding BOLD signal and store the result ( currently only
    BOLD signal ) in the hdf5 file fname_out The simulation runs in chunks to
    save working memory. chunkSize corresponds to the size in ms of a single
    chunk
    """
    ...
    while dt * idxLastT < totalDuration:
        ...
        currentChunkSize = min(
            chunkSize + delay_Ndt,
            totalDuration - dt * idxLastT + (delay_Ndt + 1) * dt,
        )
