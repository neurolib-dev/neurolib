...
def timeIntegration(params):
    ...
    Return:
      rates_exc:  N*L array   : containing the exc. neuron rates in kHz time
        series of the N nodes
      rates_inh:  N*L array   : containing the inh. neuron rates in kHz time
        series of the N nodes
      t:          L array     : the time value at which the time series are
        evaluated
    ...
    warn = params[
        "warn"
    ]  # Display a warning if out og precalc limits in interpolation,
    ...
    if N == 1:
        ...
        # Interareal connection delays, Dmat(i,j) Connnection from jth node to
        # ith (ms)
    ...
    Dmat_ndt = np.around(Dmat / dt).astype(
        int
    )  # delay matrix in multiples of dt
    ...
    dosc_version = params[
        "dosc_version"
    ]  # (Use dynamic oscillations?) if 0: exponential version (fewer dimensions, faster)
    ...
    sigmae_ext = params[
        "sigmae_ext"
    ]  # External exc input standard deviation ( mV/sqrt(ms) )
    sigmai_ext = params[
        "sigmai_ext"
    ]  # External inh input standard deviation ( mV/sqrt(ms) )

    # recurrent coupling parameters
    Ke = params[
        "Ke"
    ]  # Recurrent Exc coupling. "EE = IE" assumed for act_dep_coupling in current implementation
    Ki = params[
        "Ki"
    ]  # Recurrent Exc coupling. "EI = II" assumed for act_dep_coupling in current implementation
    ...
    tau_se = params[
        "tau_se"
    ]  # Synaptic decay time constant for exc. connections "EE = IE" (ms)
    tau_si = params[
        "tau_si"
    ]  # Synaptic decay time constant for inh. connections  "EI = II" (ms)
    ...
    Vs = params[
        "Vs"
    ]  # Cutoff or spike voltage value, determines the time of spike (mV)
    ...
    mufe = params[
        "mufe_init"
    ].copy()  # Filtered mean input (mu) for exc. population
    mufi = params[
        "mufi_init"
    ].copy()  # Filtered mean input (mu) for inh. population
    mufe_dosc = params["mufe_init"].copy() + np.zeros(N) * (
        1j
    )  # (if dosc_version: complex variable) (mV/ms)
    mufi_dosc = params["mufi_init"].copy() + np.zeros(N) * (
        1j
    )  # (if dosc_version: complex variable) (mV/ms)
    ...
    if np.shape(params["rates_exc_init"])[1] == 1:
        ...
        rates_exc = np.dot(
            params["rates_exc_init"], np.ones((1, len(t)))
        )  # kHz
        rates_inh = np.dot(
            params["rates_inh_init"], np.ones((1, len(t)))
        )  # kHz
    else:
    ...
    if distr_delay:
        ...
        for l in range(N):
            Dmat_ndt[
                ...
                0.0
            )  # if distributed, this will calculated in the r_d ODE (Eq. 4.43)
    else:
        rd_exc = np.zeros(
            (N, N)
        )  # kHz  rd_exc(i,j): Connection from jth node to ith
        rd_inh = np.zeros(N)
        ...
        for l in range(N):
            Dmat_ndt[
                ...
                ndt_de
            )  # if no distributed, this is a fixed value (E-E coupling)

    # Save the noise in the rates array to save memory
    if RNGseed:
        np.random.seed(RNGseed)
    rates_exc[:, startind:] = np.random.standard_normal(
        (N, len(range(startind, len(t))))
    )
    rates_inh[:, startind:] = np.random.standard_normal(
        (N, len(range(startind, len(t))))
    )
    ...
    for i in range(startind, len(t)):
        ...
        for no in range(N):
            ...
            if not distr_delay:
                # interareal coupling
                for l in range(N):
                    rd_exc[l, no] = rates_exc[
                        no, i - Dmat_ndt[l, no] - 1
            ...
            mue = (
                ...
                + Jei_max * seim[no]
                + mue_ext[no]
                + ext_exc_current[no, i]
            )
            mui = (
                Jie_max * siem[no]
                + Jii_max * siim[no]
                + mui_ext[no]
                + ext_inh_current[no, i]
            )
            ...
            z2ee = (
                ...
                + c_gl ** 2 * Ke_gl * ext_exc_rate[no, i]
            )
            z2ei = cei ** 2 * Ki * rd_inh[no]
            z2ie = (
                cie ** 2 * Ke * rd_exc[no, no]
                + c_gl ** 2 * Ke_gl * ext_inh_rate[no, i]
            ...
            rates_exc[no, i] = interpolate_values(
                precalc_r, xid1, yid1, dxid, dyid
            )
            Vmean_exc = interpolate_values(precalc_V, xid1, yid1, dxid, dyid)
            tau_exc = interpolate_values(precalc_tau_mu, xid1, yid1, dxid, dyid)
            tau_sigmae_eff = interpolate_values(
                precalc_tau_sigma, xid1, yid1, dxid, dyid
            )

            # ------- inhibitory population

            xid1, yid1, dxid, dyid = fast_interp2_opt(
                sigmarange, ds, sigmai_f, Irange, dI, mufi[no]
            )
            xid1, yid1 = int(xid1), int(yid1)

            rates_inh[no, i] = interpolate_values(
                precalc_r, xid1, yid1, dxid, dyid
            )
            ...
            tau_sigmai_eff = interpolate_values(
                precalc_tau_sigma, xid1, yid1, dxid, dyid
            )
            ...
            IA_rhs = (
                a * (Vmean_exc - EA) - IA[no] + tauA * b * rates_exc[no, i]
            ) / tauA
            ...
            seev_rhs = (
                (1 - seem[no]) ** 2 * z2ee
                + (z2ee - 2 * tau_se * (z1ee + 1)) * seev[no]
            ) / tau_se ** 2
            seiv_rhs = (
                (1 - seim[no]) ** 2 * z2ei
                + (z2ei - 2 * tau_si * (z1ei + 1)) * seiv[no]
            ) / tau_si ** 2
            siev_rhs = (
                (1 - siem[no]) ** 2 * z2ie
                + (z2ie - 2 * tau_se * (z1ie + 1)) * siev[no]
            ) / tau_se ** 2
            siiv_rhs = (
                (1 - siim[no]) ** 2 * z2ii
                + (z2ii - 2 * tau_si * (z1ii + 1)) * siiv[no]
            ) / tau_si ** 2
            ...
            mue_ext[no] = (
                mue_ext[no]
                + (mue_ext_mean - mue_ext[no]) * dt / tau_ou
                + sigma_ou * sqrt_dt * noise_exc[no]
            )  # mV/ms
            mui_ext[no] = (
                mui_ext[no]
                + (mui_ext_mean - mui_ext[no]) * dt / tau_ou
