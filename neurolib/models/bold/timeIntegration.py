...
def simulateBOLD(Z,dt,voxelCounts,X=None, F=None, Q=None, V=None ):
    ...
    # Note: the distribution of each Balloon-Windkessel models parameters values are given pro Voxels
    ...
    return integrateBOLD_numba(BOLD,X,Q,F,V,
                               Z, dt,N,
                               rho, alpha, V0, k1, k2, k3, Gamma, K, Tau)

@numba.njit
def integrateBOLD_numba(BOLD,X,Q,F,V,
                               Z, dt,N,
                               rho, alpha, V0, k1, k2, k3, Gamma, K, Tau):
...
#        BOLD[:,i] = V0 * ( k1 * ( 1 - Q) + k2 * (1 - Q / V ) + k3 * (1 - V) )
        ...
        for j in range(N): # loop over all areas
            ...
            Q[j] = Q[j] + dt / Tau[j] * ( F[j] / rho * (1- (1- rho)**(1/F[j])) \
                                            - Q[j] * V[j] **(1/alpha - 1 ) )
